...
function handleSocketConfig(req, res) {
    ...
    var ip6_url = Config.get("ipv6.domain") + ":" + Config.get("ipv6.port");
    res.send("var IO_URL='"+io_url+"',WEB_URL='"+web_url+"',SSL_URL='" + ssl_url +
             "',ALLOW_SSL="+Config.get("https.enabled") +
             ",IPV6_URL='"+ip6_url+"',ALLOW_IPV6="+Config.get("ipv6.enabled")+";" +
             (Config.get("ipv6.enabled") ?
             "if(USEROPTS.ipv6){IO_URL=WEB_URL=IPV6_URL}" : "") +
